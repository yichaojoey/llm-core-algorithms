...
- **第六天：大一统感知视界的分水岭 (多模态 VLM)**
  - **What**: 左右互搏 [vit_image_patching.py](./Pretraining/Multi_Modal_Vision_Language/vit_image_patching.py)、[cross_modal_projector.py](./Pretraining/Multi_Modal_Vision_Language/cross_modal_projector.py) 和压轴的 [gemini_native_multimodal.py](./Pretraining/Multi_Modal_Vision_Language/gemini_native_multimodal.py)。
  - **Why**: 如今全人类都在卷多模态大一统！面试官极其爱刁钻提问：“主流架构是怎么把外部图像和系统文字连接在一起算的？” 你当明白这世界存在两大派系：
    1. **后填补拼接怪 (Late Fusion / LLaVA系)**：使用便宜的双层线性投影仪 (`MLP Projector`) 强行把外界传来的羸弱短维图片词，硬生生撑长、拉大到跟 4096 文本维相同的空间，粗暴地利用 `Concat` 进行尾部补齐！
    2. **降维原生霸主 (Native Fusion / Gemini系)**：毫不妥协的原教旨主意。鄙视翻译官插件。在第一手抽取图片分词时，直接和文本采取完全相等的终极维度！让字与画极度疯狂地按照用户行为做交错纠缠（Interleave），并在联合自注意力层内，让文字向量天然地撞击图片向量（Joint Attention）！这才是端到端的绝顶级理解！
...
- **第十一天：操作系统的显存移植降维 (PagedAttention)**
...
- **第十二天：突破内存天堑 (Speculative Decoding)**
...
- **第十三天：集群斩裂封神阵 (Distributed Scaleouts)**
